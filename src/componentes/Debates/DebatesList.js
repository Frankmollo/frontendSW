...
const DebatesForm = ({ docenteId }) => {
  ...
  const handleChange = (e) => {
    setDebate({ ...debate, [e.target.name]: e.target.value });
  };

  const handleSubmit = (e) => {
    e.preventDefault();
    alert('Debate guardado (solo frontend): ' + JSON.stringify(debate, null, 2));
  };
    ...
    <Box sx={{ maxWidth: 480, mx: 'auto', mt: 6 }}>
      <Paper elevation={4} sx={{ p: 4, borderRadius: 4, background: '#f8fafc' }}>
        ...
        <form onSubmit={handleSubmit}>
          <TextField label="Curso" name="curso" value={debate.curso} onChange={handleChange} select fullWidth margin="normal" variant="outlined" required>
            {cursosEjemplo.map((curso) => (
              <MenuItem key={curso.id} value={curso.nombre}>{curso.nombre}</MenuItem>
            ))}
          </TextField>
          <TextField label="Título" name="titulo" value={debate.titulo} onChange={handleChange} fullWidth margin="normal" variant="outlined" required />
          <TextField label="Fecha" name="fecha" value={debate.fecha} onChange={handleChange} type="date" fullWidth margin="normal" variant="outlined" InputLabelProps={{ shrink: true }} required />
